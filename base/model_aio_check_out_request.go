...
type AioCheckOutRequest struct {
	...
	// **Client 端返回特店的按鈕連結** 消費者點選此按鈕後，會將頁面導回到此設定的網址 注意事項： 導回時不會帶付款結果到此網址，只是將頁面導回而已。 設定此參數，綠界會在付款完成或取號完成頁面上顯示[返回商店]的按鈕。 設定此參數，發生簡訊 OTP 驗證失敗時，頁面上會顯示[返回商店]的按鈕。 若未設定此參數，則綠界付款完成頁或取號完成頁面，不會顯示[返回商店]的按鈕。 若導回網址未使用 https 時，部份瀏覽器可能會出現警告訊息。
	...
	EncryptType *int32 `json:"EncryptType,omitempty"`
...
// GetEncryptType returns the EncryptType field value if set, zero value otherwise.
func (o *AioCheckOutRequest) GetEncryptType() int32 {
	if o == nil || o.EncryptType == nil {
		var ret int32
		return ret
	}
...
// GetEncryptTypeOk returns a tuple with the EncryptType field value if set, nil otherwise
// and a boolean to check if the value has been set.
...
// SetEncryptType gets a reference to the given int32 and assigns it to the EncryptType field.
